...
export default function BalancedEnterpriseHome() {
  ...
  const handleFileDrop = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      if (!validExtensions.includes(fileExtension)) {
        triggerToast(`❌ Error: Invalid file format. Please upload a valid CAD blueprint.`);
        return;
      }
  ...
  const simulateProfileDownload = () => {
    triggerToast('📄 Initializing secure transfer sequence for LASER_TECH_COMPANY_PROFILE.pdf.');
  };
  ...
  return (
    <div className="min-h-screen transition-colors duration-300" style={{ backgroundColor: '#F8F6F2', color: '#26322E' }}>
      ...
      {/* HERO WORKSPACE WITH UNIFIED COMPACT BADGES SYSTEM */}
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-10 items-center">
          ...
          <div className="lg:col-span-7 space-y-5">
            ...
            <div className="flex flex-wrap gap-2 items-center">
              ...
              <div className="px-3 py-1.5 rounded-lg text-[10px] font-mono font-black tracking-widest uppercase shadow-xs border bg-[#F3E8D6] text-[#7C5A28] border-[#D5BC8D]">
            ...
            <div className="flex flex-wrap gap-3 pt-2">
              ...
              <Link href="/ai-booth" className="px-6 py-3.5 text-white font-black text-xs uppercase tracking-wider rounded-xl shadow-md transition-all text-center transform hover:-translate-y-0.5 bg-[#C08A3E] hover:bg-[#B17C33]">
                Open Customizer Booth
              </Link>
              <button 
                type="button"
                onClick={simulateProfileDownload}
                className="hidden md:inline-block px-5 py-3.5 font-bold text-xs uppercase tracking-wider rounded-xl shadow-2xs border transition-all text-center transform hover:-translate-y-0.5 bg-transparent border-[#D5BC8D] text-[#7C5A28] hover:bg-[#F1ECE4]"
              >
                Company Profile
              </button>
            </div>
          ...
          {/* MACHINERY CAPACITY GRID NODES PANEL */}
          <div className="lg:col-span-5 bg-white border border-[#F1ECE4] p-6 rounded-3xl shadow-[0_20px_40px_rgba(38,50,46,0.06)] space-y-4">
            ...
            <div className="divide-y divide-stone-100 text-xs space-y-3 font-semibold text-stone-600">
              <div className="pt-2 flex justify-between items-center">
                <span>Heavy Duty CNC Router Bed:</span>
                <span className="font-mono bg-[#F1ECE4] px-2.5 py-0.5 rounded text-[#26322E] font-bold">8.0 × 4.0 Feet</span>
              </div>
              <div className="pt-3 flex justify-between items-center">
                <span>Industrial CO2 Laser Bed:</span>
                <span className="font-mono bg-[#F1ECE4] px-2.5 py-0.5 rounded text-[#26322E] font-bold">8.0 × 4.0 Feet</span>
              </div>
              <div className="pt-3 flex justify-between items-center">
                <span>High-Speed Fiber Metal Marker:</span>
                <span className="font-mono bg-[#F1ECE4] px-2.5 py-0.5 rounded text-[#26322E] font-bold">2.0 × 2.0 Feet</span>
              </div>
              <div className="pt-3 flex justify-between items-center font-mono text-[10px] text-stone-400 uppercase tracking-wider">
                <span>📍 Location: Mawanella</span>
      ...
      {/* TAB SEGMENTED B2B VS B2C OPERATIONAL CHANNELS */}
      <section className="py-20 px-4 max-w-7xl mx-auto space-y-8">
        ...
        <div className="max-w-4xl mx-auto pt-4">
          {activeTab === 'industrial' ? (
            <div className="grid grid-cols-1 md:grid-cols-12 gap-8 bg-white border border-[#F1ECE4] p-6 md:p-8 rounded-3xl items-center shadow-xs animate-fade-in">
              <div className="md:col-span-7 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-2 font-mono text-[10px] text-[#7C5A28] bg-[#F8F6F2] p-3 rounded-xl border border-[#D5BC8D]/40">
                  <div>• 8x4 MDF Router Carving</div>
                  <div>• QR Code Asset Serialization</div>
                  <div>• Stainless Steel Nameplates</div>
                  <div>• Floating Acrylic Signage</div>
                </div>
              </div>
              
              {/* B2B BLUEPRINT UPLOAD DISPATCH */}
              <div className="md:col-span-5 bg-[#F8F6F2] border-2 border-dashed border-stone-300 hover:border-[#C08A3E] p-6 rounded-2xl text-center relative transition-all group">
                ...
                <p className="text-[9px] font-mono text-stone-400 mt-0.5">Accepts complex .DWG, .DXF, .SVG, or engineering vector .PDF</p>
                {cadFile ? (
            ...
            <div className="bg-white border border-[#F1ECE4] p-6 md:p-8 rounded-3xl space-y-4 max-w-xl mx-auto shadow-xs text-center animate-fade-in">
              ...
              <div className="flex gap-2 justify-center pt-2">
                <Link href="/portfolio" className="px-5 py-2.5 bg-[#26322E] hover:bg-[#33423D] text-[#F8F6F2] font-bold text-xs uppercase tracking-wider rounded-xl transition-colors shadow-xs">View Finished Catalog</Link>
                <Link href="/ai-booth" className="px-5 py-2.5 bg-[#C08A3E] hover:bg-[#B17C33] text-white font-bold text-xs uppercase tracking-wider rounded-xl transition-colors shadow-xs">Open Design Studio ➔</Link>
              </div>
            </div>
          )}
        </div>
